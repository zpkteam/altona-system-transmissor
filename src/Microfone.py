...
# ----------------------------
CHUNK = 4800  # 1 segundo de áudio (samples por leitura)
...
# =============================
# FILTRO A-WEIGHTING (48 kHz)
# =============================
b0 = 0.234300125141527
b1 = -0.468600250283055
b2 = -b0
b3 = 0.937200500566110
b4 = -b0
b5 = b1
b6 = -b4
b = [b0, b1, b2, b3, b4, b5, b6]

a1 = -4.113050107329961
a2 = 6.553157702958693
a3 = -4.990918046664842
a4 = 1.785795382617397
a5 = -0.246210802686762
a6 = 0.011225878476737
a = [1, a1, a2, a3, a4, a5, a6]

# =============================
# CONSTANTES DE INTEGRAÇÃO
# =============================
tau = 1.0  # Slow = 1s (use 0.125 para FAST)
#dt = CHUNK / RATE
dt = 0.1
alpha = np.exp(-dt / tau)
...
def capturar_audio():
    ...
    # =============================
    # WARM-UP DO FILTRO (2 segundos)
    # =============================

    zi = np.zeros(max(len(a), len(b)) - 1)

    warmup_time = 2.0
    warmup_chunks = int(warmup_time / dt)

    for _ in range(warmup_chunks):
        data = stream.read(CHUNK, exception_on_overflow=False)
    
        samples32 = np.frombuffer(data, dtype=np.int32)
        samples24 = samples32 >> 8
        x = samples24.astype(np.float64) / FULL_SCALE
    
        _, zi = lfilter(b, a, x, zi=zi)
    
    print("✅ Filtro estabilizado.")
    ...
    try:
        while True:
            ...
            # Integração exponencial (Slow ou Fast)
            if dba_int is None:
                dba_int = dba
            else:
                dba_int = alpha * dba_int + (1 - alpha) * dba
